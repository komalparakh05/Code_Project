...
import Twitter from './utils/TweetContracts.json'
...
function Feed({personal}) {
  ...
  const getAllTweets = async() => {
    try {
      ...
      if(ethereum) {
        ...
        const TweetContracts = new ethers.Contract(
          TwitterContractAddress,
          Twitter.abi,
          signer
        )

        let allTweets = await TweetContracts.getAllTweets();
  ...
  useEffect(() => {
    getAllTweets(); // eslint-disable-next-line
  }, []);
 ...
 // eslint-disable-next-line
        let deleteTweetTx = await TweetContracts.deleteTweet(key, true);
        let allTweets = await TweetContracts.getAllTweets();
        setPosts(getUpdatedTweets(allTweets, ethereum.selectedAddress));
      } else {
        console.log("Ethereum object doesn't exist");
